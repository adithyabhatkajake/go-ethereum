...
struct query_response_t* query(struct file_t* file,struct query_t query_obj)
{
    ...
    for(uint32_t i = 0;i < query_obj.query_length;i++) {
        ...
        mpz_t integer;
        char* str = hexstring((unsigned char*)file->pieces[query_obj.indices[i]].data,
...
struct query_t* generate_query(uint32_t* index_array, uint32_t array_len)
{
    struct query_t* q = (struct query_t*)malloc(sizeof(struct query_t));

    q->query_length = array_len;
    q->indices = (uint32_t*)malloc(sizeof(uint32_t)*array_len);
    memcpy(q->indices,index_array,sizeof(uint32_t)*array_len);
    
    q->pairing = (struct  pairing_s*)malloc(sizeof(struct pairing_s));
    
    pairing_init_set_buf(q->pairing, a1_param, strlen(a1_param));
    q->nu = (struct element_s*)malloc(sizeof(element_t)*array_len);

    for(uint32_t i=0;i<array_len;i++)
    {
        element_init_Zr(q->nu+i,q->pairing);
        element_random(q->nu+i);
    }
