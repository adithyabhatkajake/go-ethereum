...
struct query_t* import_query(const char* filename)
{
    FILE* file = fopen(filename, "rb");
    struct query_t* q = (struct query_t*)malloc(sizeof(struct query_t));
    q->pairing = (struct pairing_s*)malloc(sizeof(pairing_t));
    
    pairing_init_set_buf(q->pairing,a1_param, strlen(a1_param));

    unsigned char* data = (unsigned char*)malloc(ZR_LEN);

    fread(&(q->query_length),sizeof(uint32_t),1,file);
    q->indices = (uint32_t*)malloc(sizeof(uint32_t)*q->query_length);
    q->nu = (struct element_s*)malloc(sizeof(element_t)*q->query_length);

    for(uint32_t i=0;i<q->query_length;i++) {
        element_init_Zr(q->nu+i,q->pairing);
        fread(q->indices+i,sizeof(uint32_t),1,file);
        fread(data,ZR_LEN,1,file);
        element_from_bytes(q->nu+i,data);
    }
...
void export_query(struct query_t* q, const char* filename)
{
    FILE* file = fopen(filename, "wb");
    unsigned char* data = (unsigned char*)malloc(ZR_LEN);

    fwrite(&(q->query_length),sizeof(uint32_t),1,file);
    for(uint32_t i=0;i<q->query_length;i++) {
        element_to_bytes(data,q->nu+i);
        fwrite(q->indices+i,sizeof(uint32_t),1,file);
        fwrite(data,ZR_LEN,1,file);
    }
